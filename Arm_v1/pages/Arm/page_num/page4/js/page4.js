function goBack() {
	const initStringData = "num_stack_exit"; // 替换实际字符串
	// 2. 调用字符串请求函数
...
function captureImage() {
    ...
    try {
        ...
        const fileName = `数块码垛${timestamp}.jpg`;
...
function initServerUrl() {
    ...
    // 构建完整的视频流URL
    serverUrl = `${serverUrlValue}:9090/stream?topic=/num_stack/image_raw`;
...
function sendStartStackCommand() {
    console.log('发送开始码垛命令: num_stack_enter');
    
    // 发送开始码垛命令到服务器
    sendCommandToServer('num_stack_enter');
...
function sendStopStackCommand() {
    console.log('发送停止码垛命令: num_stack_exit');
    
    // 发送停止码垛命令到服务器
    sendCommandToServer('num_stack_exit');

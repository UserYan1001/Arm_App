function goBack() {
	const initStringData = "num_track off"; // 替换为数字块追踪关闭命令
	// 2. 调用字符串请求函数
    ...
    if (window.plus) {
        ...
        const prevPage = plus.webview.getWebviewById('num_page0');
        ...
        if (prevPage) {
            ...
            plus.webview.create(page0Path, 'num_page0').show('slide-in-left', 300);
        }
...
function captureImage() {
    ...
    try {
        ...
        const fileName = `数块追踪${timestamp}.jpg`;
...
function initServerUrl() {
    ...
    serverUrl = `${serverUrlValue}:9090/stream?topic=/num_track/image_raw`;
...
function toggleTracking(enabled) {
    ...
    // 发送开关状态信息到服务器
    const command = enabled ? 'num_track on' : 'num_track off';
    sendTrackingCommand(command);
    if(command == 'num_track on')
	{
		console.log('刷新视频流');
		startStreaming();
	}
    console.log('数块追踪功能已' + (enabled ? '开启' : '关闭'));
}
...
function selectNumber(num) {
    ...
    if (isTrackingEnabled) {
        ...
        // 发送数字选择信息到服务器
        const command = `num_track ${num}`;
        sendTrackingCommand(command);
        
        console.log('选择了数字:', num);
    } else {
        console.log('请先开启数块追踪开关');
    }
}

// 更新数字显示框
function updateNumberDisplay() {
    const numberDisplay = document.getElementById('currentColor');
    const numberText = document.getElementById('colorText');
    
    if (selectedNumber !== null && numberMap[selectedNumber]) {
        numberDisplay.style.backgroundColor = numberMap[selectedNumber];
        numberText.textContent = `数字${selectedNumber}`;
    } else {
        numberDisplay.style.backgroundColor = 'transparent';
        numberText.textContent = '未选择';
    }
}

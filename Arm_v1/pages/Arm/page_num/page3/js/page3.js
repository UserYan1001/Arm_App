function goBack() {
	const initStringData = "sort_stop_num"; // 替换实际字符串
	// 2. 调用字符串请求函数
...
function captureImage() {
    ...
    try {
        ...
        const fileName = `数块分拣${timestamp}.jpg`;
...
function initServerUrl() {
    ...
    serverUrl = `${serverUrlValue}:9090/stream?topic=/num_sorting/image_raw`;
...
function sendSelectedColors() {
    ...
    if (document.getElementById('checkNum1').checked) {
        selectedColors.push('sort_num1');
    }
    if (document.getElementById('checkNum2').checked) {
        selectedColors.push('sort_num2');
    }
    if (document.getElementById('checkNum3').checked) {
        selectedColors.push('sort_num3');
    }
    ...
    if (selectedColors.length === 0) {
        // 显示弹窗提示
        if (window.plus) {
            plus.nativeUI.alert('未选择数字', function() {}, '提示', '确定');
        } else {
            alert('未选择数字');
        }
        return;
...
function sendStopCommand() {
    console.log('发送停止分拣命令: sort_stop_num');
    
    // 直接调用sendSortCommand发送停止命令
    sendSortCommand('sort_stop_num');
...
document.addEventListener('DOMContentLoaded', function() {
	...
	const initStringData = "num_sorting_enter"; // 替换实际字符串
	// 2. 调用字符串请求函数
	sendStringToServer(initStringData);
    // 检查HTML5+环境
	console.log('刷新视频流');

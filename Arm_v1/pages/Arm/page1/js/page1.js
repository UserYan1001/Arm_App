...
function handleButtonDown(jointIndex, step) {
    if (!isPressed[jointIndex]) {
        ...
        // 立即执行一次
        adjustJointValue(jointIndex, step);
        
        // 设置定时器实现长按连续增长
        longPressTimers[jointIndex] = setInterval(() => {
            if (isPressed[jointIndex]) {
                adjustJointValue(jointIndex, step);
            }
        }, 50); // 每50毫秒执行一次
...
function resetAllJoints() {
    for (let i = 0; i < 6; i++) {
        ...
        if (valueElement) {
            valueElement.textContent = '1500';
        }
        
        // 发送复位命令
        sendJointCommand(i, 1500);
    }
...
function sendJointCommand(jointIndex, value) {
    ...
    const commandUrl = `${baseUrl}/joint`;
...
function initServerUrl() {
    ...
    if (!baseUrl) {
        baseUrl = 'http://192.168.0.105'; // 默认备用地址
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        if (minusBtn) {
            minusBtn.addEventListener('touchstart', () => handleButtonDown(i, -1));
            minusBtn.addEventListener('touchend', () => handleButtonUp(i));
        ...
        if (plusBtn) {
            plusBtn.addEventListener('touchstart', () => handleButtonDown(i, 1));
            plusBtn.addEventListener('touchend', () => handleButtonUp(i));

function goBack() {
	const initStringData = "label_track_off"; // 替换实际字符串
	// 2. 调用字符串请求函数
    ...
    if (window.plus) {
        ...
        const prevPage = plus.webview.getWebviewById('code_page0');
        ...
        if (prevPage) {
            ...
            plus.webview.create(page0Path, 'code_page0').show('slide-in-left', 300);
        }
...
function captureImage() {
    ...
    try {
        ...
        const fileName = `码块追踪${timestamp}.jpg`;
...
function initServerUrl() {
    ...
    serverUrl = `${serverUrlValue}:9090/stream?topic=/label_track/image_raw`;
...
function toggleTracking(enabled) {
    ...
    // 发送开关状态信息到服务器
    const command = enabled ? 'label_track_on' : 'label_track_off';
    sendTrackingCommand(command);
	if(command == 'label_track_on')
	{
		console.log('刷新视频流');
		startStreaming();
	}
    
    console.log('码块追踪功能已' + (enabled ? '开启' : '关闭'));
}
...
function selectColor(label) {
    ...
    if (isTrackingEnabled) {
        ...
        // 发送码块选择信息到服务器
        const command = `track ${label}`;
        sendTrackingCommand(command);
        
        console.log('选择了码块:', label);
    } else {
        console.log('请先开启码块追踪开关');
    }
}
...
function updateColorDisplay() {
    ...
    if (selectedLabel && labelMap[selectedLabel]) {
        colorDisplay.style.backgroundColor = labelMap[selectedLabel];
        ...
        colorText.textContent = labelName;
    } else {

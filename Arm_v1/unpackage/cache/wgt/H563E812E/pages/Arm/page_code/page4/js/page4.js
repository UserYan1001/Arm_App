function goBack() {
	const initStringData = "label_stack_exit"; // 替换实际字符串
	// 2. 调用字符串请求函数
...
function captureImage() {
    ...
    try {
        ...
        const fileName = `码块码垛${timestamp}.jpg`;
...
function initServerUrl() {
    ...
    serverUrl = `${serverUrlValue}:9090/stream?topic=/label_stack/image_raw`;
...
function sendStartStackCommand() {
    console.log('发送开始码垛命令: label_stack_enter');
    
    // 发送开始码垛命令到服务器
    sendCommandToServer('label_stack_enter');
    console.log('刷新视频流');
    startStreaming();
    // 显示开始码垛提示
    showToast('已发送开始码垛命令');
...
function sendStopStackCommand() {
    console.log('发送停止码垛命令: label_stack_exit');
    
    // 发送停止码垛命令到服务器
    sendCommandToServer('label_stack_exit');

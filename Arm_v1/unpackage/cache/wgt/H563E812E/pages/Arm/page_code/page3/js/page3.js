function goBack() {
	const initStringData = "sort_stop_code"; // 替换实际字符串
	// 2. 调用字符串请求函数
...
function captureImage() {
    ...
    try {
        ...
        const fileName = `码块分拣${timestamp}.jpg`;
...
function initServerUrl() {
    ...
    // 构建完整的视频流URL
    serverUrl = `${serverUrlValue}:9090/stream?topic=/label_sorting/image_raw`;
...
function sendSelectedColors() {
    ...
    if (document.getElementById('check1').checked) {
        selectedColors.push('sort_1');
    }
    if (document.getElementById('check2').checked) {
        selectedColors.push('sort_2');
    }
    if (document.getElementById('check3').checked) {
        selectedColors.push('sort_3');
    }
    ...
    if (selectedColors.length === 0) {
        // 显示弹窗提示
        if (window.plus) {
            plus.nativeUI.alert('未选择码块', function() {}, '提示', '确定');
        } else {
            alert('未选择码块');
        }
        return;
...
function sendStopCommand() {
    console.log('发送停止分拣命令: sort_stop_code');
    
    // 直接调用sendSortCommand发送停止命令
    sendSortCommand('sort_stop_code');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const initStringData = "label_sorting_enter"; // 替换实际字符串
    // 2. 调用字符串请求函数
    sendStringToServer(initStringData);

function goBack() {
	const initStringData = "sort stop color"; // 替换实际字符串
	// 2. 调用字符串请求函数
...
function captureImage() {
    ...
    try {
        ...
        const fileName = `色块分拣${timestamp}.jpg`;
...
function initServerUrl() {
    ...
    // 构建完整的视频流URL
    serverUrl = `${serverUrlValue}:9090/stream?topic=/color_sorting/image_raw`;
...
function sendStopCommand() {
    console.log('发送停止分拣命令: sort stop color');
    
    // 直接调用sendSortCommand发送停止命令
    sendSortCommand('sort stop color');
...
document.addEventListener('DOMContentLoaded', function() {
	...
	const initStringData = "color_sorting_enter"; // 替换实际字符串
	// 2. 调用字符串请求函数
	sendStringToServer(initStringData);
	console.log('刷新视频流');
	startStreaming();

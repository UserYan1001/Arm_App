function goBack() {
	const initStringData = "color_stack_exit"; // 替换实际字符串
	// 2. 调用字符串请求函数
...
function captureImage() {
    ...
    try {
        ...
        const fileName = `色块码垛${timestamp}.jpg`;
...
function initServerUrl() {
    ...
    // 构建完整的视频流URL
    serverUrl = `${serverUrlValue}:9090/stream?topic=/color_stack/image_raw`;
...
function sendStartStackCommand() {
    console.log('发送开始码垛命令: color_stack_enter');
    
    // 发送开始码垛命令到服务器
    sendCommandToServer('color_stack_enter');
    console.log('刷新视频流');
    startStreaming();
    // 显示开始码垛提示
    showToast('已发送开始码垛命令');
...
function sendStopStackCommand() {
    console.log('发送停止码垛命令: color_stack_exit');
    
    // 发送停止码垛命令到服务器
    sendCommandToServer('color_stack_exit');
...
function sendCommandToServer(command) {
    ...
    let serverUrlValue = document.SERVER_URL;
    const commandUrl = `${serverUrlValue}:8080/control`; // 使用与其他页面相同的控制接口
